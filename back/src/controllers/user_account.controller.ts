...
export const findListAskingToBeFriend = async (req: Request, res: Response): Promise<Response<UserAccount.IUserAccount[]>> => {
  try {
    const id = getIdUserAccountInToken(req.headers.authorization!);
    const { rows } = await pool.query(
      `
...
export const findListWantToAddMe = async (req: Request, res: Response): Promise<Response<UserAccount.IUserAccount[]>> => {
  try {
    const id = getIdUserAccountInToken(req.headers.authorization!);
    const { rows } = await pool.query(
      `
